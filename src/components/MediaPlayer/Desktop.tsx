...
import { formatVideoTime, subtitleProxy } from "../../Helpers/misc";
...
const Desktop: FC<PlayerProps> = ({ playerKey, sources, subtitles }) => {
  const [quality, setQuality] = useState(0);
  const [playbackSpeed, setPlaybackSpeed] = useState(
    Number(localStorage.getItem("film-speed")) || 1
  );
  ...
  const [volume, setVolume] = useState(
    isNaN(parseInt(localStorage.getItem("film-volume") as string))
      ? 100
      : Number(localStorage.getItem("film-volume"))
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem("film-volume", String(volume));
  }, [volume]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("film-speed", String(playbackSpeed));
  ...
  return (
    <div className="relative w-[70vw] h-0 pb-[56.25%]">
      <div
        onMouseMove={() => {
        ...
        ref={containerRef}
        className="absolute top-0 left-0 flex items-center justify-center w-full h-full bg-black group"
      >
        <ReactHlsPlayer
          crossOrigin=""
          playsInline
        ...
        {loading && !paused && (
          <div className="absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer top-1/2 left-1/2">
            <div className="w-12 h-12 border-4 border-white rounded-full border-r-transparent animate-spin"></div>
          </div>
        )}

        {paused && (
          <div
            className="absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer top-1/2 left-1/2"
            onClickCapture={handleScreenClicked}
          >
            <BsFillPlayFill className="text-7xl" />
          </div>
        )}
        ...
        >
          ...
          <div className="flex items-stretch justify-between flex-grow px-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-row items-stretch volume-container">
                ...
                <div className="flex items-center justify-end w-0 overflow-hidden transition-all duration-300 volume">
                  <input
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                <div
                  ...
                  className={`absolute bottom-[40px] right-[-40px] w-[200px] h-[200px] overflow-y-auto overflow-x-hidden bg-mainBg rounded-md p-4 invisible opacity-0 transition ${
                ...
                >
                  ...
                  <h1 className="mt-4 text-lg">Subtitle</h1>
                  ...
                  <h1 className="mt-4 text-lg">Speed</h1>
